...
var Validator = function() {
  ...
  validateContext = function(obj) {
    if (!_.has(obj, "@context")) {
      ...
      if (contexts.indexOf(obj["@context"]) == -1) {
  ...
  validateObject = function(obj, path) {
    if (!_.isString(obj.name) && !_.isObject(obj.nameMap)) {
      notes.push(new Note(Note.WARNING, path, "Object instances SHOULD have a 'name' or 'nameMap' property."));
    }
    if (!_.isString(obj.id) && !_.isString(obj['@id'])) {
      notes.push(new Note(Note.NOTICE, path, "Object should have an 'id' property."));
    }
  ...
  validateItem = function(obj, path) {
    ...
    if (!type) {
      ...
      var validateFn = validateFunction(type);
      if (!validateFn) {
        notes.push(new Note(Note.INFO, path, "Object of unrecognized type " + type));
      } else {
        validateFn(obj, path);
      }
    }
  ...
  this.validateHTTPResponse = function(url, response) {
    var ct = response.headers['content-type'];
    if (!ct) {
      notes.push(new Note(Note.ERROR, [], "No Content-Type header defined for HTTP response."));
    } else if (ct != 'application/activity+json' &&
      ct != 'application/ld+json; profile="https://www.w3.org/ns/activitystreams"') {
      notes.push(new Note(Note.ERROR, [], "Document served with incorrect Content-Type header: " + ct));
    }
  };
